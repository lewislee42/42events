...
export default function IndoorHobbyForm() {
	...
	return (
		<form className="mt-6 border-2 p-4 rounded-sm border-purple-500 max-w-3xl mx-auto">
			<h5 className="text-xl my-3 bg-purple-500 text-white p-2 rounded-sm">Indoor Hobby Events Form<SiEpicgames className="inline ml-2"/></h5>
			...
			<div class="grid md:grid-cols-2 md:gap-6">
				...
				<div class="relative z-0 w-full mb-6 group">
					...
					<textarea id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-sm border border-gray-300 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500" placeholder="Additional info..."></textarea>
				</div>
			</div>
			<div class="grid md:grid-cols-2 md:gap-6">
				<div class="relative z-0 w-full mb-6 group">
			...
			<div className="my-6">
				<h3 className="text-2xl">
					<button type="button" onClick={()=>{if (engagement === false) setEngagement(true); else setEngagement(false)}} class="bg-purple-500 text-white rounded-sm flex items-center justify-between w-full py-3 px-3 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700" data-accordion-target="#accordion-engagement">
